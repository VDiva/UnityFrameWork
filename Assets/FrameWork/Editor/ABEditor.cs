...
namespace FrameWork
{
    ...
    public class ABEditor : UnityEditor.Editor
    {
        ...
        {
            {"com.unity.addressables", "ADDRESSABLESCN_INSTALLED"},
        ...
        private static void CheckFileInfo(FileInfo fileInfo,string dicPath,string dicName,string abName="other")
        {
            ...
            var dicMd5 = Tool.GetMd5AsString(dicPath);
            if (string.IsNullOrEmpty(dicMd5))return;
            var group = setting.FindGroup(dicMd5);
            if (group == null)
            {
                group=setting.CreateGroup(dicMd5,false, false, false, new List<AddressableAssetGroupSchema> { setting.DefaultGroup.Schemas[0] }, typeof(SchemaType));
            }
